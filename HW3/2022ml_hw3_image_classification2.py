...
class FoodDataset(Dataset):
    ...
    def __init__(self,path,tfm=test_tfm,files = None):
        ...
        if files != None:
            self.files = files
        print(f"One {path} sample2",self.files[0])
        self.transform = tfm
...
for fold, (train_ids, validate_ids) in enumerate(kfold.split(np.arange(len(dataset)))):

    train_subsampler = SubsetRandomSampler(train_ids)
    validate_subsampler = SubsetRandomSampler(validate_ids)
    trainloader = DataLoader(dataset, batch_size=batch_size, num_workers= 0, sampler=train_subsampler)
    validatloader = DataLoader(dataset, batch_size=batch_size, num_workers= 0, sampler=validate_subsampler)
    ...
    for epoch in range(n_epochs):
        ...
        if valid_acc > best_acc:
            with open(f"./{_exp_name}_log2.txt","a"):
        ...
        if valid_acc > best_acc:
            ...
            torch.save(model.state_dict(), f"{_exp_name}_best2.ckpt") # only save best to prevent output memory exceed error
            best_acc = valid_acc
            stale = 0
        else:
            stale += 1
...
model_best.load_state_dict(torch.load(f"{_exp_name}_best2.ckpt"))
model_best.eval()
prediction = []
with torch.no_grad():
